                    ...
                    <div class="-m-1.5 overflow-x-auto">
                        <div class="p-1.5 min-w-full inline-block align-middle">
                            <div class="grid grid-cols-4 gap-4">
                                ...
                                <div class="py-3 px-4">
                                    <form method="GET" id="" class="display: inline" action="{{ route('events.index') }}">
                            ...
                            <div class="overflow-hidden">
                                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead>
                                        ...
                                        <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">Descripción</th>
                                        <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">Fecha</th>
                                        <th scope="col" class="px-6 py-3 text-start text-xs font-medium text-gray-500 uppercase">Tickets Disponibles</th>
                                    ...
                                    <tbody>
                                        @forelse($events->data as $event)
                                            <tr class="odd:bg-white even:bg-gray-100 dark:odd:bg-slate-900 dark:even:bg-slate-800">
                                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $event->name  }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ $event->description  }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ date('d-m-Y',strtotime($event->date))  }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800 dark:text-gray-200">{{ $event->available_tickets  }}</td>
                                                <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium">
                                                    ...
                                                    <form method="POST" id="" class="display: inline" action="{{ route('events.destroy', $event->id) }}">
                                                        {{csrf_field()}}
                            ...
                            <div class="py-1 px-4">
                              <nav class="flex items-center space-x-1">
                                    ...
                                    @for ($i = 0; $i < $events->last_page; $i++)
                                        <a href="{{$events->path . '?page=' . $i + 1}}">
